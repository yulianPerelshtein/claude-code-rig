...
Self-contained: writes JSONL logs under the active project's
.claude/data/logs/<session>/ when CLAUDE_PROJECT_DIR is set, else under
~/.claude/data/logs/<session>/.
"""
...
# session_id is used to build a log path; treat it as untrusted input.
_SAFE_SESSION = re.compile(r"^[A-Za-z0-9_-]+$")


def ensure_session_log_dir(session_id: str) -> Path:
    """Ensure and return the log directory for a session.

    ``session_id`` is treated as untrusted: anything outside ``[A-Za-z0-9_-]``
    (e.g. an absolute path or ``../`` components) falls back to ``"unknown"`` so
    the log path can never escape the logs directory.
    """
    if not _SAFE_SESSION.match(session_id or ""):
        session_id = "unknown"
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR")
    base = Path(project_dir) / ".claude" if project_dir else Path.home() / ".claude"
    log_dir = base / "data" / "logs" / session_id
