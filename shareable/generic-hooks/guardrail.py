...
def load_blocklist() -> list[tuple[str, str]]:
    """Load blocked patterns from blocked-commands.json next to this file.

    Falls back to the deployed path ~/.claude/hooks/blocked-commands.json, then
    to an empty list (fail-open) so a missing config never blocks all commands.
    """
    candidates = [
        Path(__file__).resolve().parent / "blocked-commands.json",
...
def main() -> None:
    ...
    if tool == "Bash":
        ...
        # Drop this block if you do not run under WSL.
        if "/mnt/c/" in command:
            print(
